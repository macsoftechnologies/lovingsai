            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form action="<?= base_url() ?>sales/update/<?php echo $sales['sales_id'];?>" role="form" id="userForm" name="userForm" method="post" enctype="multipart/form-data">
                                <div class="form-row">
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            <label for="date">Date</label>
                                            <input type="text" name="date" id="name" class="form-control" placeholder="Enter date" value="<?php echo $sales['date'];?>"><?php echo form_error('date'); ?>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-md-3">
                                        <div class="form-group">
                                           ...
                                           <!--  <select class="form-control" name="patient_id">
                                            ...
                                            <input type="text" name="patient_name" id="patient_name" class="form-control" placeholder="Enter Patient Name" value="<?php echo $sales['patientName'];?>"><?php echo form_error('patient_name'); ?>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="InvoiceNo" id="InvoiceNo" class="form-control" placeholder="Enter InvoiceNo" value="<?php echo $sales['InvoiceNo'];?>"><?php echo form_error('InvoiceNo'); ?>
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            <label for="status">status</label>
                                            <input type="text" name="status" id="status" class="form-control" placeholder="Enter status" value="<?php echo $sales['status'];?>"><?php echo form_error('status'); ?>
                                        </div>
                                    </div>
                                               ...
                                               <table id="faqs" class="table table-hover">
                                                   ...
                                                   <tbody>
                                                    <?php if($log->num_rows() > 0) {
                                                      foreach ($log->result() as $key => $lg) { ?>
                                                       <tr id="rowdelete<?php echo $key+1;?>">
                                                           <td><input type="text" class="form-control" name="product[]" id="product" placeholder="Enter product..." value="<?php echo $lg->product;?>"></td>
                                                           <td><input type="text" class="form-control" name="quantity[]" id="quantity<?php echo $key+1;?>" placeholder="Enter quantity..." value="<?php echo $lg->quantity;?>"></td>
                                                           <td><input type="text" class="form-control" id="price<?php echo $key+1;?>" name="price[]" placeholder="Enter price" value="<?php echo $lg->price;?>"></td>
                                                           <td><input type="text" class="form-control" id="total<?php echo $key+1;?>" name="total[]" placeholder="Enter total" value="<?php echo $lg->total;?>"></td>
                                                           <td class="mt-10"><a class="badge badge-danger" style="color:#fff;"mid="<?php echo $key+1;?>" onclick="$('#rowdelete<?php echo $key+1;?>').remove();"><i class="fa fa-trash"></i> Delete</a></td>
                                                       </tr>
                                                     <?php } 
                                                   } ?>
                                                   </tbody>
...
<script type="text/javascript">
  $(document).ready(function () {
    $('body').on("change",'[id^=price]',function(e){
        var pro_id = $(this).attr('id');
        var laboratory_id = this.value;

        var txtId= "#quantity"+pro_id[pro_id.length-1]
        var txtIds= "#total"+pro_id[pro_id.length-1]
         $(txtId).empty();
         $(txtIds).empty();
         $('#systems').empty();
         var price = $(txtId).val();
    ...
    $('body').on("change",'[id^=quantity]',function(e){
        var pro_id = $(this).attr('id');
        var laboratory_id = this.value;

        var txtId= "#price"+pro_id[pro_id.length-1]
        var txtIds= "#total"+pro_id[pro_id.length-1]
         $(txtId).empty();
         $(txtIds).empty();
         $('#systems').empty();
...
<script type="text/javascript">
    $(document).ready(function () {
      for (var i = 0; i < 20; i++){
        ...
        var txtIdp= "#price"+[i]
        var txtIdq= "#quantity"+[i]
        var txtIdt= "#total"+[i]
      $(txtId).change(function(){
            $(txtIdt).empty();
            // $('#systems1').empty();

            var quantity = $(txtIdq).val();
            var price = $(txtIdp).val();
            var tot_amount = parseFloat(price) * parseFloat(quantity);
            // var total = $('#systems1').val();
            // var finalamount = parseInt(total) + parseInt(tot_amount);
            console.log(total);
            console.log(tot_amount);
            console.log(finalamount);
            // if(tot_amount == 0) {
            //    $('#systems1').val(tot_amount);
            // }else {
            //    $('#systems1').val(finalamount); 
            // }
                 
            $(txtIdt).val(tot_amount);
